...
export function MonthlySummaryPage() {
  ...
  const showFullSummary = hasFeature('monthly_summary_full');
    ...
    <div>
      ...
      <Card>
        ...
        {expensesByCategory.length === 0 ? (
          <EmptyState
            eyebrow="Lectura por categoría"
            title="Todavía no hay gastos para leer este mes"
            description="Cuando el hogar registre gastos, este bloque empezará a mostrar en qué se está yendo el dinero y dónde conviene mirar con más atención."
          ...
          <Card>
            ...
            {(() => {
              ...
              return total > 0 ? (
                ...
                <EmptyState
                  title="Todavía no hay suficiente contexto para leer el reparto"
                  description="Esta comparación aparece cuando el hogar empieza a registrar gastos compartidos y personales en el mismo mes."
                  action={{ label: 'Ver movimientos', onClick: () => navigate(`/app/movimientos?month=${year}-${String(month).padStart(2, '0')}`) }}
                />
              );
            })()}
          </Card>
